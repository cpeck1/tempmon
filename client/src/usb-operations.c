...
{
  enum ftdi_bits_type btype; /*= BITS_8;*/
  enum ftdi_stopbits_type sbits; /* = STOP_BIT_1; */
  enum ftdi_parity_type parity; /* = NONE; */
  int err;
  ...
  err = ftdi_set_line_property(ctx, btype, sbits, parity);
  if (err) {
